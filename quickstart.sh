...
# .env mínimo (app inicia sem validação, adicione secrets depois)
...
EFI_CERT_PATH=server/certs/efibank-production.p12
FIREBASE_ADMIN_CREDENTIAL_FILE=server/certs/firebase-adminsdk.json
ENVEOF
...
pm2 start node_modules/.bin/tsx --name volatuspay -- server/index.ts
pm2 save
...
echo "✅ VolatusPay no ar!"
pm2 status
curl -s http://localhost:3000/_health 2>/dev/null || echo "Aguardando app..."
